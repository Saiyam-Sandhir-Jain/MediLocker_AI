...
df_tr = pd.read_csv('chatbot/Medical_dataset/Training.csv')
df_tt = pd.read_csv('chatbot/Medical_dataset/Testing.csv')
...
knn_clf = joblib.load('chatbot/model/knn.pkl')
...
def getDescription():
    ...
    with open('chatbot/Medical_dataset/symptom_Description.csv') as csv_file:
...
def getSeverityDict():
    ...
    with open('chatbot/Medical_dataset/symptom_severity.csv') as csv_file:
...
def getprecautionDict():
    ...
    with open('chatbot/Medical_dataset/symptom_precaution.csv') as csv_file:
        csv_reader = csv.reader(csv_file, delimiter=',')
        line_count = 0
        for row in csv_reader:
...
def get_bot_response():
    ...
    if session["step"] == "age":
        try:
            ...
            return "Invalid age. Please enter a numeric value for your age."
        session["step"] = "gender"
        return "Can you specify your gender? (male or female)"
    if session["step"] == "gender":
        valid_genders = ["male", "female"]
        if s.lower() not in valid_genders:
            return "Invalid gender. Please specify 'male', 'female'."
        session["gender"] = s
        session["step"] = "Depart"
    ...
    if session['step'] == "BYE":
        ...
        if s.lower() == "yes":
            ...
            return "THANKS Mr/Ms " + name + " for using me. If you wish to call an ambulance contact <b> 108</b>"
